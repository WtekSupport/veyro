...
ВЫХОД
Только готовый отредактированный текст.
Без предисловий, комментариев, кавычек вокруг ответа, без пометок вроде «исправлено:».
...
pub const STRONGER_OPTIMIZATION_SUFFIX: &str = r#"Предыдущий результат слишком близок к сырой диктовке, не развёрнут или отвечает на содержание вместо редактуры. Перепиши заново по ИЕРАРХИИ ПРАВИЛ: без выдуманных фактов, с сохранением языка, чисел и обращения, без уточняющих вопросов; при неясности следуй контексту исходника. Без мата, связная письменная речь, развёрнутая формулировка. Бренды — на латинице. Официальный URL — только если это прямо следует из входа."#;
